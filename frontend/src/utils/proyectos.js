import img1 from "../multimedia/slide1.jpg";
import img2 from '../multimedia/build.jpg'
const proyectos = [
  {
    name: "Casa VA L3S5",
    date: "25 noviembre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img: img1,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/2022/11/banner_ache_arquitectos_casa_VA_L3S5_7.png",
    ],
    title:
      "Es una vivienda unifamiliar ubicada en Aldora Villa Familiar, Yerba Buena, Tucumán.",
    description: [
      "Este proyecto arquitectónico se destaca por sus amplios espacios integrados, grandes ventanales como testigos del imponente paisaje del cerro San Javier. Y la clara y armónica composición volumétrica, que remite a un estilo, evidentemente clásico y de porte francés. La belleza y la simpleza de un diseño único adaptado al contexto y la cualidad de nuestros clientes.",
    ],
  },
  {
    name: "HOSTERÍA DE ESCABA",
    date: "18 octubre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img:img2,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/202…he_arquitectos_remodelacion_hosteria_escaba_8.png",
    ],
    title:
      "HOSTERÍA DE ESCABA / REFUNCIONALIZACIÓN Y REMODELACIÓN",
    description: [
      "Apunta a brindar un espacio con servicios de calidad para tucumanos y turistas que deseen pasar un día o más en el destino. Así, se erigirá un alojamiento que destaque el contacto con la naturaleza y el disfrute de los paisajes que posee Escaba. Además, albergará y promoverá diferentes actividades deportivas y recreativas del lugar, con equipamiento adecuado, apto para diversos eventos al aire libre.",
    ],
  },
  {
    name: "Casa VA L3S5",
    date: "25 noviembre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img: img1,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/2022/11/banner_ache_arquitectos_casa_VA_L3S5_7.png",
    ],
    title:
      "Es una vivienda unifamiliar ubicada en Aldora Villa Familiar, Yerba Buena, Tucumán.",
    description: [
      "Este proyecto arquitectónico se destaca por sus amplios espacios integrados, grandes ventanales como testigos del imponente paisaje del cerro San Javier. Y la clara y armónica composición volumétrica, que remite a un estilo, evidentemente clásico y de porte francés. La belleza y la simpleza de un diseño único adaptado al contexto y la cualidad de nuestros clientes.",
    ],
  },
  {
    name: "HOSTERÍA DE ESCABA",
    date: "18 octubre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img:img2,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/202…he_arquitectos_remodelacion_hosteria_escaba_8.png",
    ],
    title:
      "HOSTERÍA DE ESCABA / REFUNCIONALIZACIÓN Y REMODELACIÓN",
    description: [
      "Apunta a brindar un espacio con servicios de calidad para tucumanos y turistas que deseen pasar un día o más en el destino. Así, se erigirá un alojamiento que destaque el contacto con la naturaleza y el disfrute de los paisajes que posee Escaba. Además, albergará y promoverá diferentes actividades deportivas y recreativas del lugar, con equipamiento adecuado, apto para diversos eventos al aire libre.",
    ],
  },
  {
    name: "Casa VA L3S5",
    date: "25 noviembre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img: img1,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/2022/11/banner_ache_arquitectos_casa_VA_L3S5_7.png",
    ],
    title:
      "Es una vivienda unifamiliar ubicada en Aldora Villa Familiar, Yerba Buena, Tucumán.",
    description: [
      "Este proyecto arquitectónico se destaca por sus amplios espacios integrados, grandes ventanales como testigos del imponente paisaje del cerro San Javier. Y la clara y armónica composición volumétrica, que remite a un estilo, evidentemente clásico y de porte francés. La belleza y la simpleza de un diseño único adaptado al contexto y la cualidad de nuestros clientes.",
    ],
  },
  {
    name: "HOSTERÍA DE ESCABA",
    date: "18 octubre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img:img2,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/202…he_arquitectos_remodelacion_hosteria_escaba_8.png",
    ],
    title:
      "HOSTERÍA DE ESCABA / REFUNCIONALIZACIÓN Y REMODELACIÓN",
    description: [
      "Apunta a brindar un espacio con servicios de calidad para tucumanos y turistas que deseen pasar un día o más en el destino. Así, se erigirá un alojamiento que destaque el contacto con la naturaleza y el disfrute de los paisajes que posee Escaba. Además, albergará y promoverá diferentes actividades deportivas y recreativas del lugar, con equipamiento adecuado, apto para diversos eventos al aire libre.",
    ],
  },
  {
    name: "Casa VA L3S5",
    date: "25 noviembre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img: img1,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/2022/11/banner_ache_arquitectos_casa_VA_L3S5_7.png",
    ],
    title:
      "Es una vivienda unifamiliar ubicada en Aldora Villa Familiar, Yerba Buena, Tucumán.",
    description: [
      "Este proyecto arquitectónico se destaca por sus amplios espacios integrados, grandes ventanales como testigos del imponente paisaje del cerro San Javier. Y la clara y armónica composición volumétrica, que remite a un estilo, evidentemente clásico y de porte francés. La belleza y la simpleza de un diseño único adaptado al contexto y la cualidad de nuestros clientes.",
    ],
  },
  {
    name: "HOSTERÍA DE ESCABA",
    date: "18 octubre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img:img2,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/202…he_arquitectos_remodelacion_hosteria_escaba_8.png",
    ],
    title:
      "HOSTERÍA DE ESCABA / REFUNCIONALIZACIÓN Y REMODELACIÓN",
    description: [
      "Apunta a brindar un espacio con servicios de calidad para tucumanos y turistas que deseen pasar un día o más en el destino. Así, se erigirá un alojamiento que destaque el contacto con la naturaleza y el disfrute de los paisajes que posee Escaba. Además, albergará y promoverá diferentes actividades deportivas y recreativas del lugar, con equipamiento adecuado, apto para diversos eventos al aire libre.",
    ],
  },
  {
    name: "Casa VA L3S5",
    date: "25 noviembre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img: img1,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/2022/11/banner_ache_arquitectos_casa_VA_L3S5_7.png",
    ],
    title:
      "Es una vivienda unifamiliar ubicada en Aldora Villa Familiar, Yerba Buena, Tucumán.",
    description: [
      "Este proyecto arquitectónico se destaca por sus amplios espacios integrados, grandes ventanales como testigos del imponente paisaje del cerro San Javier. Y la clara y armónica composición volumétrica, que remite a un estilo, evidentemente clásico y de porte francés. La belleza y la simpleza de un diseño único adaptado al contexto y la cualidad de nuestros clientes.",
    ],
  },
  {
    name: "HOSTERÍA DE ESCABA",
    date: "18 octubre, 2022",
    place: "Tucuman",
    address: "Aldora Villa Familiar, Yerba Buena, Tucumán.",
    img:img2,
    photos: [
        "https://achearquitectos.com/wp-content/uploads/202…he_arquitectos_remodelacion_hosteria_escaba_8.png",
    ],
    title:
      "HOSTERÍA DE ESCABA / REFUNCIONALIZACIÓN Y REMODELACIÓN",
    description: [
      "Apunta a brindar un espacio con servicios de calidad para tucumanos y turistas que deseen pasar un día o más en el destino. Así, se erigirá un alojamiento que destaque el contacto con la naturaleza y el disfrute de los paisajes que posee Escaba. Además, albergará y promoverá diferentes actividades deportivas y recreativas del lugar, con equipamiento adecuado, apto para diversos eventos al aire libre.",
    ],
  },
];

export default proyectos;
